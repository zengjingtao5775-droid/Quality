...
def build_qwen_quality_prompt(report_scope: str, language: str, prompt_profile: str) -> tuple[str, str]:
    ...
    if prompt_profile == "zx_conclusion":
        ...
        fixed_schema = (
            ...
            "(2) '## 2. PS 已做行动' with columns 'CC | DKL FQC 记录 | DKL 抽检量 | 首次 RFT（PASS/有效记录） | 未通过（FAIL/NG/重验）记录 | 最近 FQC'; "
            ...
            "(2) '## 2. Completed PS Actions' with columns 'CC | DKL FQC Records | DKL Sampled Qty | First-Pass RFT (PASS/valid) | Not Passed (FAIL/NG/recheck) | Latest FQC'; "
...
def zx_conclusion_title(facts: dict, language: str) -> str:
    del facts
    return "AI 质量结论报告（ZX-R12M）" if language == "中文" else "AI Quality Conclusion Report (ZX-R12M)"
...
def zx_conclusion_has_canonical_scope(content: str, facts_json: str, language: str) -> bool:
    ...
    required = (
        ["高风险 CC Top 5", "PS 已做行动", "推荐行动计划", "DPU（疵点数/检验数）"]
        if language == "中文"
        else ["Top 5 High-Risk CCs", "Completed PS Actions", "Recommended Action Plan", "DPU (defect quantity / inspected)"]
    )
...
def build_zx_conclusion_report(facts_json: str, language: str, narrative: dict | None = None) -> str:
    ...
    if language == "中文":
        ...
        for index, item in enumerate(products, start=1):
            ...
            action_rows.append(
                f"| {cc} | {number(action.get('fqc_records'))} | {number(action.get('fqc_sampled'))} | "
...
## 2. PS 已做行动
| CC | DKL FQC 记录 | DKL 抽检量 | 首次 RFT（PASS/有效记录） | 未通过（FAIL/NG/重验）记录 | 最近 FQC |
|---|---:|---:|---:|---:|---|
...
| CC | DKL FQC Records | DKL Sampled Qty | First-Pass RFT (PASS/valid) | Not Passed (FAIL/NG/recheck) | Latest FQC |
|---|---:|---:|---:|---:|---|
