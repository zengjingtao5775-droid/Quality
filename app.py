...
def build_qwen_quality_prompt(report_scope: str, language: str, prompt_profile: str) -> tuple[str, str]:
    ...
    if prompt_profile == "zx_conclusion":
        ...
        fixed_schema = (
            ...
            "(2) '## 2. PS&工厂已做行动' with columns 'CC | PS FQC 记录 | 工厂 FQC 记录 | 首次 RFT（PASS/有效记录） | 最近 FQC'; "
            ...
            "(2) '## 2. Completed PS & Factory Actions' with columns 'CC | PS FQC Records | Factory FQC Records | First-Pass RFT (PASS/valid) | Latest FQC'; "
        ...
        return (
            ...
            f"{fixed_schema} "
            "Do not add an executive summary. Section 2 must show PS and factory FQC record counts and first-pass RFT for each CC. Section 3 must contain exactly three concise actions tied to the Top 5 CCs, each with a clear completion standard. "
...
def build_zx_conclusion_report(facts_json: str, language: str, narrative: dict | None = None) -> str:
    ...
    if language == "中文":
        ...
        if recommended_actions:
            ...
            action_plan_rows = (
                f"| 1 | DKL FQC 跟进 | {failed_ccs} | 完成下一轮 FQC，并在报告中更新首次 RFT |\n"
...
## 2. PS&工厂已做行动
| CC | PS FQC 记录 | 工厂 FQC 记录 | 首次 RFT（PASS/有效记录） | 最近 FQC |
|---|---:|---:|---:|---|
...
"""
    ...
    if recommended_actions:
        ...
        action_plan_rows = (
            f"| 1 | DKL FQC follow-up | {failed_ccs} | Complete the next FQC round and update first-pass RFT |\n"
...
| CC | PS FQC Records | Factory FQC Records | First-Pass RFT (PASS/valid) | Latest FQC |
|---|---:|---:|---:|---|
