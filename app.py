...
# AI action reporting combines the three PS inspectors with all factory
# inspectors, while these two named inspectors are excluded from both groups.
ZX_AI_ACTION_EXCLUDED_INSPECTOR_PATTERNS = (
    "韩永红",
    "李秀玲",
)
...
def build_tu_community_ai_fact_pack(
    ...
    for index, product in enumerate(product_facts[:5], start=1):
        ...
        if not cc_fqc.empty:
            cc_fqc["_action_inspector"] = cc_fqc.get(
                "inspector", pd.Series("", index=cc_fqc.index)
            ).map(normalize_zx_inspector_name)
            excluded_mask = cc_fqc["_action_inspector"].map(
                lambda name: any(pattern in name for pattern in ZX_AI_ACTION_EXCLUDED_INSPECTOR_PATTERNS)
            )
            cc_fqc = cc_fqc[~excluded_mask].copy()
            cc_fqc["_action_owner"] = cc_fqc["_action_inspector"].map(
                lambda name: (
                    "PS"
                    if any(pattern in name for pattern in ZX_DECATHLON_PO_SAMPLING_INSPECTOR_PATTERNS)
                    else "ZX Factory"
                )
            )
        ...
        cp_processes = (
            " / ".join(
                dict.fromkeys(
                    value
                    for value in cp_for_cc.get("process", pd.Series(dtype=object)).fillna("").astype(str).str.strip()
                    if value
                )
            )[:180]
        ...
        cp_control_points = (
            " / ".join(
                dict.fromkeys(
                    value
                    for value in cp_for_cc.get("control_point", pd.Series(dtype=object)).fillna("").astype(str).str.strip()
                    if value
                )
            )[:220]
            if not cp_for_cc.empty
        ...
        ps_action_facts.append(
            {
                ...
                "ps_fqc_records": int(cc_fqc["_action_owner"].eq("PS").sum()) if not cc_fqc.empty else 0,
                "factory_fqc_records": int(cc_fqc["_action_owner"].eq("ZX Factory").sum()) if not cc_fqc.empty else 0,
...
def build_qwen_quality_prompt(report_scope: str, language: str, prompt_profile: str) -> tuple[str, str]:
    ...
    if prompt_profile == "zx_conclusion":
        ...
        return (
            ...
            f"{fixed_schema} "
            "Do not add an executive summary. Section 2 must show Decathlon and factory FQC record counts and first-pass RFT for each CC. Section 3 is rendered deterministically by the application from the Top 5 risk facts, linked CP/process facts, and dynamic AQL recommendations. "
...
def build_zx_conclusion_report(facts_json: str, language: str, narrative: dict | None = None) -> str:
    ...
    if language == "中文":
        ...
        for index, item in enumerate(products, start=1):
            ...
            action_rows.append(
                f"| {cc} | {number(action.get('ps_fqc_records'))} | {number(action.get('factory_fqc_records'))} | "
                f"{rft_text} | {action.get('latest_fqc_date') or '-'} |"
            )
        action_plan_rows = []
        for index, item in enumerate(products, start=1):
            cc = str(item.get("cc") or "-")
...
## 2. Decathlon & 工厂已做行动
| CC | Decathlon FQC 记录 | 工厂 FQC 记录 | 首次 RFT（PASS/有效记录） | 最近 FQC |
|---|---:|---:|---:|---|
...
## 2. Completed Decathlon & Factory Actions
| CC | Decathlon FQC Records | Factory FQC Records | First-Pass RFT (PASS/valid) | Latest FQC |
|---|---:|---:|---:|---|
...
def render_qwen_summary_panel(
    ...
    *,
    ...
    if prompt_profile == "zx_conclusion":
        ...
        hero_title = "AI 质量结论报告（ZX-R12M）" if active_language == "中文" else "AI Quality Conclusion Report (ZX-R12M)"
        hero_subtitle = (
            "聚焦高风险 CC Top 5、FQC 记录，以及推荐行动计划。"
            if active_language == "中文"
            else "Focused on the Top 5 high-risk CCs, FQC records, and a recommended action plan."
        )
...
def prepare_fixed_product_risk(products: pd.DataFrame) -> pd.DataFrame:
    ...
    factory_code = view["factory_code"].fillna("").astype(str).str.strip()
    product_code = view["product_code"].fillna("").astype(str).str.strip()
    view["product_view"] = np.where(factory_code.eq("ZX"), product_code, factory_code + " / " + product_code)
    view["product_label_display"] = view["product_label"].map(localize_product_label)
...
def render_zx_cc_defect_rate_trend_v1(finished_df: pd.DataFrame, products: pd.DataFrame) -> None:
    ...
    fig.update_traces(
        ...
        hovertemplate=(
            ...
            f"{t('疵点数', 'Defects')}  %{{customdata[2]:,.0f}}"
            "<extra></extra>"
        ),
    )
    ...
    if scope_key == "ZX":
        ...
        with st.expander(t("更多分析", "More Analysis"), expanded=True):
            ...
            if selected_analysis == "process":
                ...
                st.markdown(
                    f"**{'AI 质量结论报告（ZX-R12M）' if report_language == '中文' else 'AI Quality Conclusion Report (ZX-R12M)'}**"
                )
                render_qwen_summary_panel(
                    "zx_dashboard1_conclusion",
                    "AI 质量结论报告（ZX-R12M）" if report_language == "中文" else "AI Quality Conclusion Report (ZX-R12M)",
...
def _render_bme_bike_quality_dashboard_v3_legacy(events: pd.DataFrame) -> None:
...
def render_bme_bike_quality_dashboard_v3(
    ...
    """BME management + engineering cockpit grounded in each source's semantics."""
    if events.empty:
        st.error(t("未读取到 BME 数据，请检查 BME Database 文件夹。", "No BME data was loaded. Check the BME Database folder."))
        return
    suppliers = sorted(events["supplier"].dropna().astype(str).unique())
    ...
    if product_issues.empty:
        ...
        product_summary = product_gate.groupby(
            ["product_key", "product_label", "product_group"], as_index=False
        ).agg(
            affected_gates=("quality_gate", "nunique"),
            defect_qty=("defect_qty", "sum"),
        ...
        # Rate denominators must include every valid inspection row for the
        # same product, not only the rows that contain a defect. Otherwise a
        # defect-only numerator view would inflate the displayed rate.
        product_denominators = all_product_master.groupby(
            ["supplier", "product_key", "product_label", "product_group", "quality_gate"], as_index=False
        ).agg(
            inspected_qty=("inspected_qty", lambda values: pd.to_numeric(values, errors="coerce").sum(min_count=1)),
        )
        chart_product_gate = chart_product_gate.merge(
            product_denominators,
            on=["supplier", "product_key", "product_label", "product_group", "quality_gate"],
            how="left",
            validate="one_to_one",
        )
        chart_product_gate["defect_rate"] = np.where(
            chart_product_gate["inspected_qty"].fillna(0).gt(0),
            chart_product_gate["defect_qty"] / chart_product_gate["inspected_qty"],
            np.nan,
        )
        gate_meta = {
            "IQC": (
        ...
        for supplier_name in ["CMW", "FSD", "TEKTRO"]:
            ...
            if not supplier_gate_charts and not supplier_gate_notes:
            ...
            gate_specs: list[list[dict[str, object] | None]] = []
            ...
            for chart_index, (gate_name, gate_top, chart_color) in enumerate(supplier_gate_charts):
                ...
                if supplier_name == "CMW" and gate_name == "IQC":
                    ...
                    gate_hovertemplate = (
                        ...
                        f"{t('不良率', 'Defect Rate')}  %{{customdata[7]:.2%}}<br>"
                        f"{t('不良记录数', 'Defect Records')}  %{{customdata[8]:,.0f}}<br>"
                        f"{t('最近检验日期', 'Latest Inspection Date')}  %{{customdata[9]}}<extra></extra>"
                    )
                ...
                if supplier_name == "CMW":
                    ...
                    if rate_rows.empty:
                        combined_gate_fig.add_annotation(
                            text=t("缺少各车型生产量或检验量，暂不能计算不良率", "Model-level production or inspection quantity is unavailable; defect rate cannot be calculated"),
                            x=0.5,
                            y=0.5,
                            showarrow=False,
                            font=dict(size=15, color="#667085"),
            ...
            for note_gate, note_rows in supplier_gate_notes:
                note_total = float(note_rows["defect_qty"].sum())
        ...
        top_issue_names = issue_summary.head(30)["issue_driver"]
...
if active_scope_key == "BME_CMW":
    render_bme_bike_quality_dashboard_v3(bme_events, bme_customer_nc, bme_fsd_orders)
    st.stop()
