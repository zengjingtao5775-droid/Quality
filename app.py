...
def build_qwen_quality_prompt(report_scope: str, language: str, prompt_profile: str) -> tuple[str, str]:
    ...
    if prompt_profile == "zx_conclusion":
        ...
        fixed_schema = (
            ...
            "(2) '## 2. PS&工厂已做行动' with columns 'CC | PS FQC 记录 | 工厂 FQC 记录 | FQC 抽检量 | 首次 RFT（PASS/有效记录） | 最近 FQC'; "
            "(3) '## 3. 推荐行动计划' with columns '序号 | 行动 | 重点 CC | 完成标准'."
            if language == "中文"
            ...
            "(2) '## 2. Completed PS & Factory Actions' with columns 'CC | PS FQC Records | Factory FQC Records | FQC Sampled Qty | First-Pass RFT (PASS/valid) | Latest FQC'; "
            "(3) '## 3. Recommended Action Plan' with columns 'No. | Action | Priority CCs | Completion Standard'."
        )
        return (
            ...
            f"{fixed_schema} "
            "Do not add an executive summary. Section 2 must show DKL FQC records, sampled quantity, and first-pass RFT for each CC. Section 3 must contain exactly three concise actions tied to the Top 5 CCs, each with a clear completion standard. "
...
def zx_conclusion_has_canonical_scope(content: str, facts_json: str, language: str) -> bool:
    ...
    required = (
        ["高风险 CC Top 5", "PS&工厂已做行动", "推荐行动计划", "DPU（疵点数/检验数）"]
        if language == "中文"
        else ["Top 5 High-Risk CCs", "Completed PS & Factory Actions", "Recommended Action Plan", "DPU (defect quantity / inspected)"]
    )
    return all(token in content for token in required) and all(cc in content for cc in expected_ccs if cc)


ZX_CONCLUSION_AI_PROMPT_VERSION = "zx-conclusion-v6-factory-actions"
...
def build_zx_conclusion_report(facts_json: str, language: str, narrative: dict | None = None) -> str:
    ...
    if language == "中文":
        ...
        for index, item in enumerate(products, start=1):
            ...
            action_rows.append(
                f"| {cc} | {number(action.get('ps_fqc_records'))} | {number(action.get('factory_fqc_records'))} | {number(action.get('fqc_sampled'))} | "
        ...
        if recommended_actions:
            ...
            action_plan_rows = (
                f"| 1 | DKL FQC 跟进 | {failed_ccs} | 完成下一轮 FQC，并在报告中更新抽检量和首次 RFT |\n"
...
## 2. PS&工厂已做行动
| CC | PS FQC 记录 | 工厂 FQC 记录 | FQC 抽检量 | 首次 RFT（PASS/有效记录） | 最近 FQC |
|---|---:|---:|---:|---:|---|
...
"""
    ...
    if recommended_actions:
        ...
        action_plan_rows = (
            f"| 1 | DKL FQC follow-up | {failed_ccs} | Complete the next FQC round and update sampled quantity and first-pass RFT |\n"
...
## 2. Completed PS & Factory Actions
| CC | PS FQC Records | Factory FQC Records | FQC Sampled Qty | First-Pass RFT (PASS/valid) | Latest FQC |
|---|---:|---:|---:|---:|---|
